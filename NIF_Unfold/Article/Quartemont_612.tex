...
% Technically the technique is least squares, the code or program is STAYSL
...
\IEEEPARstart{C}{haracterizing} the energy dependence of a neutron environment has many applications to the nuclear sciences community. 
...
% This is a great overview, but provide road markers (i.e. Section III outlines...)
...
\label{fig:Spec1}
\end{figure}
% The error bars don't look to be under 1.5%.  Differential would have been helpful due to the different bin widths (or using equal bin widths).  Comparing fig 2 and 3, I wonder is there is an error in the input source.
...
\begin{figure}[h!]
	...
	\label{fig:NIFSRC}
\end{figure}
% You don't need the label if there is one line on the plot.  
...
\section{Description of Work}
	...
	\:dE ,
	\end{equation}
	% I changed it previously, but you changed it back - I think it should be \Sigma_{act}(E) instead of \Sigma(E)_{act}.  The subscript should be tied to the variable (sigma) that it modifies 
...
The activation of the foil for a given	irradiation time ($t_{i})$ is given as a function of the decay constant: 
	...
	% In general, I would say you can eliminate this section unless there is a driving motivation for the half-life (like you can't measure the foils for a long time or you are able to measure them instantaneously).   
...
% Check out McFarland in Amy's Mendeley repo for other options
...
\end{equation}
% What about the live time fraction?  
...
% The truth hurts sometimes...However, this fit looks much better, so the truth is not always painful :)  Still missing that odd low - energy tail, but that can be hard to capture.
...
% You did put Bq/g as your input in STAYSL, right? I just ask becuse it isn't listed in the table (which is fine) but other STAYSL relevant details are. 
\begin{table}[]
...
%How did you determine how many to put in a bin? Interpolation?	
...
% If we map over the statistical and data uncertainties as discussed in the prospectus today, we avoid some of these arbitrary decisions (and hopefully it works!). 
...
% The chi-squared convergence is much lower than any value I have ever used.  It is somewhat surprising to me that this is such a difficult problem to tackle - so much for a clean, well-defined source.
...
\begin{figure}[t!]	
	...
	\label{fig:kbas}
\end{figure}
% It may just be my screen resolution, but make sure to save the images with at least 600 DPI - they are better and it will save your from having to regenerate them later!
% It almost seems that the labels are swapped as to whether sigma was updated.  The sigma updated spectrum is a more drastic change from the original spectrum in the first two cases, contrary to expectations.
...
%The table headers should be bolded and/or distinguished in some way
\begin{table}[h]
